...
def bloom_b3ds(blob3dlist, stitch=False, create_progress_bar=True):
    ...
    for bnum, blob2d in enumerate(allb2ds): # HACK need to put the colon on the right of start_offset
        # printl(' Blooming b2d: ' + str(bnum) + '/' + str(len(allb2ds)) + ' = ' + str(blob2d), flush=True)
        blob2d.bloomInwards() # NOTE will have len 0 if no blooming can be done
        pb.update(len(blob2d.pixels), set=False) # set is false so that we add to an internal counter
        # printl('DB pb symbols = ' + str(pb.symbols_printed), flush=True)

    pb.finish()

    print_elapsed_time(t_start_bloom, time.time(), prefix='took')
    printl('Before blooming there were: ' + str(num_unbloomed) + ' b2ds contained within b3ds, there are now ' + str(len(Blob2d.all)))
...
def main():
    ...
    if not Config.dePickle:
        ...
        # printl('Plotting simple children of non_beads one by one..')
        # for nb in non_beads:
        #     printl(nb)
        #     printl(' ' + str(nb.get_first_child_beads()))
        ...
        #
        # printl('Now plotting all b3ds')
        # plotBlob3ds(list(Blob3d.all.values()))
        # printl('Plotting all but base b3ds, which assists with viewing')
